...
class PlayerViewController: UIViewController,AVAudioPlayerDelegate {
    ...
    override func viewDidLoad() {
        ...
        setupUI()
        if viewTitle == "audio" {
            self.lblTitleHeading.text = "Please wait while audio is playing..."
            setupAudio()
        }
        else if viewTitle == "video" {
            self.lblTitleHeading.text = "Please wait while video is playing..."
            setupVideo()
        }
        else if viewTitle == "image" {
                   self.lblTitleHeading.text = "Please check the image below.."
                   setupImage()
               }
    }
    ...
    func setupImage() {
        self.audioClipIcon.image = UIImage(named: resourceName)
        self.timer.invalidate()
        var timertime = 1.0
        self.lblTimer.text = "0/5"
        self.timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true, block: { (kTimer) in
            ...
            self.progressBar.setProgress(Float(timertime/Double(5)), animated: true)
            self.lblTimer.text = "\(Int(timertime))/\(5))"
